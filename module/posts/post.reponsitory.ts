...
async findByIdAggregate(postId: string, userId: string, page = 1, limit = 10) {
     ...
     const pipeline: PipelineStage[] = [
          {
               $match: { _id: postObjectId }, // ép về ObjectId để match post
          },
          {
               $lookup: {
                    from: "feels",
                    let: { postIdStr: { $toString: "$_id" } }, // ép _id của post sang string
                    pipeline: [
                         {
                              $match: {
                                   $expr: { $eq: ["$postId", "$$postIdStr"] }, // so sánh string với string
                              },
                         },
                              ...
                              vars: {
                                   myFeel: {
                                        $first: {
                                             $filter: {
                                                  input: "$feels",
                                                  cond: { $eq: ["$$this.userId", userId] }, // userId là string nên so sánh trực tiếp
                                             },
                                        },
                                   },
                              },
                                   ...
                                   vars: {
                                        myFeel: {
                                             $first: {
                                                  $filter: {
                                                       input: "$feels",
                                                       cond: { $eq: ["$$this.userId", userId] }, // userId cũng là string
                                                  },
                                             },
                                        },
                                   },
